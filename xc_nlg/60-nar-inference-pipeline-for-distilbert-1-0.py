...
n_proc, n_lbl, n_data = 100, block.n_lbl, block.test.dset.data.n_data
...
# %% ../nbs/60-nar-inference-pipeline-for-distilbert.ipynb 17
...
for o in output:
    ...
    data.extend(d)
    indices.extend(i)
    indptr.extend([len(x) for x in d])

data, indices, indptr = np.hstack(data), np.hstack(indices), np.hstack([np.zeros(1, dtype=np.int64), np.array(indptr).cumsum(axis=0)])
scores = csr_matrix((data, indices, indptr), shape=(n_data, n_lbl))

from xclib.data.data_utils import write_sparse_file

data_dir = '/home/scai/phd/aiz218323/scratch/outputs/60-nar-inference-pipeline-for-distilbert'
fname = f'{data_dir}/wikiseealso-bm25-output.bow'
write_sparse_file(scores, fname)
